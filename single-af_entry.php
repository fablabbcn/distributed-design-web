...
$fields = array(
	...
	array( 'details' ),
	array( 'images', 'video' ),
	array( 'contact_details', 'website', 'social_media', 'acceptance' ),
);
		...
		<?php foreach ( array_merge( ...$fields ) as $field ) : ?>
			<section class="<?php the_classes( $s_classes['section'] ); ?>">
				...
				<div data-layout="<?php echo esc_attr( get_row_layout() ); ?>" class="<?php the_classes( $s_classes['layout'] ); ?>">
					<div class="<?php the_classes( $s_classes['columns'](false)[0] ); ?>"><?php include locate_template( 'template-parts/singular/details.php' ); ?></div>
