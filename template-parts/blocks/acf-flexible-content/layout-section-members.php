...
<div class="px-5 py-10 md:px-10 xl:p-20">
    ...
    <div class="grid-members-section grid grid-cols-1 lg:grid-cols-2 gap-x-10">
        ...
        <?php foreach($members_section_members as $member): ?>
            ...
            <div data-membernum="<?php echo $num_member; ?>" class="col-member cursor-pointer grid grid-cols-12 gap-x-2 relative border-b py-2 <?php echo ($num_member < 2) ? 'lg:border-t' : '' ?> <?php echo ($num_member < 1) ? 'border-t' : '' ?>">
            ...
            <div class="fixed z-50 px-10 py-20 bg-white h-screen left-0 top-0 opacity-0 transition-opacity duration-500 ease-in-out pointer-events-none" data-membermap="<?php echo $num_member; ?>">
